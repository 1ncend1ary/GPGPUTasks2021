...
//    // Менее красивый ракурс, но в этом ракурсе виден весь фрактал:
...

    {
        ...
        // stored in cmake-build-debug directory
    ...
    context.activate();
    {
        ...
        bool printLog = false;
        kernel.compile(printLog);  // nothing in Build Log
      ...
      gpu_results_gpu.resizeN(width * height);  // видеопамять выделена
      // should store values : str1+str2+str3+...+strn
      // every line of image should be stored consequently

      {
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
//          mandelbrotCPU(gpu_results.ptr(), width, height, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit, false);
          unsigned int workGroupSizeX = 128;
          unsigned int workGroupSizeY = 1;
          // я не верю что картинка узкая, поэтому WG будет 128x1
          // 2048 это очень даже широко, но, может быть, потом будет другая ширина картинки.
          // тем не менее, я рассчитываю, что больше 128 она точно будет.
          unsigned int global_work_size_x = (width + workGroupSizeX - 1) / workGroupSizeX * workGroupSizeX;
          unsigned int global_work_size_y = height;
          kernel.exec(gpu::WorkSize(workGroupSizeX, workGroupSizeY, global_work_size_x, global_work_size_y), gpu_results_gpu, width, height, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit);
